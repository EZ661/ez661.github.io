...
### 打包本地模组为整合包
6. 在tml中将自己要玩的即上传到服务器的模组全部启用，确认配置好后，在模组管理页面点击将已启用模组保存为模组整合包，然后打开创建好的模组整合包的文件夹，切换到Mod和Modconfig两个文件夹的目录，将这两个文件夹打包压缩，建议压缩为7z。
7. 打包之后再上传，启动项改两个指定模组和模组配置文件夹路径的代码，然后解压模组。